...
# crosswise validation of model
  ...
  for (test_period in 1:n_valid_periods)
  {
    ...
    test_index[test_index_from:test_index_to] = (mydata$julian_day[test_index_from:test_index_to] >=152) &
                                                (mydata$julian_day[test_index_from:test_index_to] <=350)       #define test period
    ...
    if (nrow(mydata_train_x) == 0 | nrow(mydata_test_x) == 0)
      next                                                    #skip this year if training or test dataset is empty
